...
HeadBumpParticle::HeadBumpParticle(Graphics& graphics, units::Game center_x, units::Game center_y) :
	...
	sprite_(
			graphics, "Caret",
			units::gameToPixel(kSourceX), units::gameToPixel(kSourceY),
			units::gameToPixel(kWidth), units::gameToPixel(kHeight)
	       ),
...
bool HeadBumpParticle::update(units::MS elapsed_time_ms) {
	particle_a_.magnitude = std::min(particle_a_.magnitude + kSpeed * elapsed_time_ms, max_offset_a_);
	particle_b_.magnitude = std::min(particle_b_.magnitude + kSpeed * elapsed_time_ms, max_offset_b_);
